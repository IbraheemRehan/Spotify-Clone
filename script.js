...
async function loadPlaylistByIndex(index) {
    ...
    for (const song of songs) {
        if (!song) {
            console.warn("Skipping undefined song in playlist:", songs);
            continue;
        }
...
const playMusic = (track, pause = false) => {
    ...
    document.querySelector(".songinfo").innerHTML = decodeURIComponent(cleanTrack.split("/").pop());
...
async function displayAlbums() {
    ...
    Array.from(document.getElementsByClassName("card")).forEach(card => {
        card.addEventListener("click", async (event) => {
            ...
            for (const song of songs) {
                if (!song) {
                    console.warn("Undefined song in album load", songs);
                    continue;
                }
